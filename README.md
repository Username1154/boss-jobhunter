# 🤖 Boss JobHunter — 全平台自动求职，睡觉时也在投简历
...
| 平台 | 方式 | 支持功能 |
|------|------|------|
| 🟢 **Boss直聘** | API（轻量，无需浏览器） | 搜索/投递/回复/反问 |
| 🟢 **Boss直聘** | CDP浏览器（更稳定） | 搜索/投递/回复 |
| 🔵 **猎聘** | 浏览器自动化 | 搜索/投递/消息检查 |
| 🟡 **前程无忧** | 浏览器自动化 | 搜索/投递（当前反爬，暂时禁用） |
...
# 2. F12 → Application → Cookies → zhipin.com
# 3. 复制 __zp_stoken__ 等关键 cookie
...
```json
{
  ...
  "auto_apply": false,              // ⚠️ 初次先关
  "auto_reply": false,              // ⚠️ 初次先关
  "job_preferences": {
    "titles": ["品牌策划", "内容策略"],
    "min_salary": 8000,
    "cities": ["深圳", "东莞"]
...
```
boss-jobhunter/
├── main.py              # Boss直聘 API 主循环
├── multi_hunter.py      # 全平台浏览器模式（猎聘+前程无忧+Boss）
├── zhipin.py            # Boss直聘 API 适配器（逆向工程）
├── filter.py            # 岗位/公司评估引擎
